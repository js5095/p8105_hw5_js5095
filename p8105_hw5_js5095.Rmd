...
## Generate 7 lists with μ={0,1,2,3,4,5,6}. Each list contains 5000 datasets of n=30, with fixed {\sigma} = 5.
...
for(i in 0:6){
  ...
  all_test_data2[[i+1]] = t_test_1sample_batch3(all_sim_data[[i+1]])
  all_test_data2[[i+1]] = bind_rows(all_test_data2[[i+1]])
}
```

## Plot 1. the proportion of times the null was rejected (the power of the test) on the y axis and the true value of μ on the x axis.
...
for(i in 0:6){
  temp_rej = tibble(
    mu = i,
    prop_rej = sum(all_test_data2[[i+1]][2]<0.05)/5000
  ) 
...
## Plot 2. the average estimate of μ̂  on the y axis and the true value of μ on the x axis.

```{r}
avg_mu_hat = tibble()
for(i in 0:6){
  mean = all_test_data2[[i+1]] %>%
    select(mu_h) %>%
